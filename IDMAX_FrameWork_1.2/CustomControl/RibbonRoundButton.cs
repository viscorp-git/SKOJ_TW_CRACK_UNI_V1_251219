...
namespace IDMAX_FrameWork
{
    public class RibbonRoundButton : Button
    {
        ...
        protected override void OnPaint(PaintEventArgs pevent)
        {
            ...
            FillBeizer(g, r, 1, ColorOver,ColorBase);
            DrawImage(g, r, ImgOffset, ImgScale);
            DrawStroke(g, r, 1, ColorStroke, GetLightColor());
        ...
        int i_mode = 0; //0 Entering, 1 Leaving, 2 Pressing, 3 Up
        int i_factor = 20;
        ...
        void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (i_mode == 2)
            {
                if (System.Math.Abs(ColorPress.R - R0) > i_factor)
                { i_fR = i_factor; }
                else { i_fR = 1; }
                if (System.Math.Abs(ColorPress.G - G0) > i_factor)
                { i_fG = i_factor; }
                else { i_fG = 1; }
                if (System.Math.Abs(ColorPress.B - B0) > i_factor)
                { i_fB = i_factor; }
                else { i_fB = 1; }
                ...
                else if (ColorPress.B > B0)
                {
                    B0 += i_fB;
                }

                if (ColorPress == Color.FromArgb(R0, G0, B0))
                {
                    timer1.Stop();
                }
                else
